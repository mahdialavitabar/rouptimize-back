  ...
  imports: [
    ConfigModule.forRoot({
      ...
      validationSchema: Joi.object({
        // Database: accepts DATABASE_URL or DB_*/PG* individual vars
        DATABASE_URL: Joi.string().uri().optional(),
        DB_HOST: Joi.string().when('DATABASE_URL', {
          is: Joi.exist(),
          then: Joi.optional(),
          otherwise: Joi.string().when('PGHOST', {
            is: Joi.exist(),
            then: Joi.optional(),
        ...
        DB_USERNAME: Joi.string().when('DATABASE_URL', {
          is: Joi.exist(),
          then: Joi.optional(),
          otherwise: Joi.string().when('PGUSER', {
            is: Joi.exist(),
            then: Joi.optional(),
            otherwise: Joi.required(),
          }),
        ...
        DB_DATABASE: Joi.string().when('DATABASE_URL', {
          is: Joi.exist(),
          then: Joi.optional(),
          otherwise: Joi.string().when('PGDATABASE', {
            is: Joi.exist(),
            then: Joi.optional(),
            otherwise: Joi.required(),
          }),
        ...
        // Redis: accepts REDIS_URL or REDIS_HOST/REDISHOST individual vars
